...
<template>
  <div :class="[ui.wrapper, to && ui.to]" v-bind="attrs" :style="{ '--color-light': colorLight, '--color-dark': colorDark }">
    ...
    <strong class="mr-2 text-[--color-light] dark:text-[--color-dark]">Documentation in Progress</strong><br/>
    <div class="pl-6">
      <ContentSlot :use="$slots.default" unwrap="p">Wiki migration currently in progress</ContentSlot>
    </div>
  </div>
</template>