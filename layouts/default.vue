...
const category = navKeyFromPath(route.path.slice(0, route.path.lastIndexOf('/')), 'category', navigation.value);

// Grab root parent title/icon as fallbacks
const parentPath = route.path.split('/').at(1);
const parentTitle = parentPath ? navKeyFromPath(`/${parentPath}`, 'title', navigation.value) : undefined;
const parentIcon = parentPath ? navKeyFromPath(`/${parentPath}`, 'icon', navigation.value) : undefined;
